...
from submission.reproduce_checker import (
    check_reproducibility,
    get_reproduce_script_path,
)
from submission.generate_representations import (
...
def _save_model(predictor: ImmuneStatePredictor, out_dir: str, train_dir: str) -> None:
    ...
    model_path = os.path.join(
        out_dir, f"{os.path.basename(train_dir)}_ensemble_model.pkl"
    )
...
def _generate_representations_direct(
    data_dir: str, dataset_type: str, config: RepresentationConfig
):
...
def _aggregate_representations_direct(
    data_dir: str, dataset_name: str, dataset_type: str, config: AggregateConfig
):
    ...
    for bert_method in config.bert_pooling_methods:
        ...
        for row_pooling_method in config.row_pooling_methods:
            ...
            output_dir = os.path.join(
                config.aggregate_out_dir, f"aggregated_esm2_t6_8M_{bert_method}"
            )
            ...
            if os.path.exists(output_path):
                ...
                try:
                    ...
                    print(
                        f"  ⚠️  Warning: Existing file appears corrupted, will regenerate: {e}"
                    )
            ...
            try:
                if dataset_type == "train":
                    ...
                    if "repertoire_id" in df_metadata.columns:
                        ...
                        df_metadata["repertoire_id"] = df_metadata[
                            "filename"
                        ].str.replace(".tsv", "", regex=False)
                        id_col = "repertoire_id"
                    else:
                        raise ValueError(
                            f"Cannot find ID column in metadata. Columns: {list(df_metadata.columns)}"
                        )

                    df_labels = df_metadata.set_index(id_col)["label_positive"].astype(
                        int
                    )
                    ...
                    for rep_id in df_labels.index:
                        filepath = os.path.join(
                            features_dir, f"{rep_id}_embeddings.npz"
                        )
                        ...
                        with np.load(filepath) as data:
                            if embedding_key not in data:
                                print(
                                    f"    Warning: Key '{embedding_key}' not found in {filepath}. Available keys: {list(data.keys())}"
                                )
                                continue
                            embeddings = data[embedding_key]
                            aggregated_vector = apply_pooling(
                                embeddings, row_pooling_method
                            )
                    ...
                    if len(repertoire_features) == 0:
                        raise RuntimeError(
                            f"No ESM2 feature files loaded. Expected files like: {os.path.join(features_dir, '{rep_id}_embeddings.npz')}"
                        )
                    ...
                    if (
                        dataset_num is not None
                        and config.sample_submissions_path is not None
                    ):
                        # Standard test dataset with sample_submissions.csv
                        from submission.aggregate_representations import (
                            load_test_dataset_embeddings,
                        )

                        X_features, sample_ids, npz_files = (
                            load_test_dataset_embeddings(
                        ...
                        print(
                            f"    Loading custom test dataset (no sample_submissions.csv)..."
                        )

                        sample_embeddings = []
                        sample_ids = []
                        npz_files = sorted(
                            glob.glob(os.path.join(features_dir, "*_embeddings.npz"))
                        )
                        ...
                        for npz_file in npz_files:
                            sample_id = os.path.basename(npz_file).replace(
                                "_embeddings.npz", ""
                            )
                            with np.load(npz_file) as data:
                                if embedding_key not in data:
                                    print(
                                        f"    Warning: Key '{embedding_key}' not found in {npz_file}. Available keys: {list(data.keys())}"
                                    )
                                    continue
                                embeddings = data[embedding_key]
                                aggregated_vector = apply_pooling(
                        ...
                        if len(sample_embeddings) == 0:
                            raise RuntimeError(
                                f"No valid embeddings found in {features_dir}"
                            )
                ...
                try:
                    ...
                    raise RuntimeError(
                        f"Failed to verify saved aggregate: {output_path}"
                    )
    ...
    if all_npz_files_to_remove and all_aggregates_successful:
        ...
        print(f"{'=' * 70}")
        print(
            f"Expected {len(expected_aggregates)} aggregates, got {len(successful_aggregates)} successful"
        )
        print(f"Keeping {len(all_npz_files_to_remove)} npz files for potential reuse")
        print(
            f"Tip: Rerun to complete remaining aggregates, then npz files will be cleaned up"
        )
    else:
        print(
...
def _ensure_representations_exist(
    data_dir: str,
    dataset_type: str,
    out_dir: str,
    use_esm: bool = True,
    ...
    if (
        not os.path.exists(rep_dir)
        or len(glob.glob(os.path.join(rep_dir, "*.npz"))) == 0
    ):
    ...
    try:
        _aggregate_representations_direct(
            data_dir, dataset_name, dataset_type, agg_config
        )
    except Exception as e:
...
def main(
    ...
    # Check train dataset
    _ensure_representations_exist(
        train_dir, "train", out_dir, use_esm, num_gpus, esm_batch_size
    )

    # Check test datasets
    for test_dir in test_dirs:
        _ensure_representations_exist(
            test_dir, "test", out_dir, use_esm, num_gpus, esm_batch_size
        )

    # Check for reproducibility - if input matches a known Kaggle dataset
    if use_reproduce:
        kaggle_reproduce_dir = os.path.join(
            os.path.dirname(__file__), "kaggle_reproduce"
        )
        matched_dataset = check_reproducibility(
            train_dir, test_dirs, kaggle_reproduce_dir
        )
    else:
    ...
    if matched_dataset:
        ...
        if os.path.exists(reproduce_script):
            ...
            cmd = (
                ...
                + test_dirs
                + ["--out_dir", out_dir, "--n_jobs", str(n_jobs)]
            )
    ...
    predictor = ImmuneStatePredictor(
        ...
        rank_topseq=topseq,
    )  # instantiate with any other parameters as defined by you in the class
...
def run():
    ...
    parser.add_argument(
        ...
        action="store_true",
        default=False,
        help="Run only the top sequence lookup (rankseq_lookup.py) instead of full pipeline",
    )
    args = parser.parse_args()

    if args.top_seq_only:
        from submission.rankseq_lookup import main as rankseq_main
