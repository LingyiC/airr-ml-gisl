...
# MODIFIED: Added specific_kmers parameter
def load_and_encode_kmers_combined(data_dir: str, k_list: List[int] = [3, 4], 
                                   min_kmer_count: int = 2,
                                   batch_size: int = 100,
                                   specific_kmers: Set[str] = set()) -> Tuple[pd.DataFrame, pd.DataFrame]:
    """
    ...
    Args:
        ...
        specific_kmers: Set of k-mers to include regardless of min_kmer_count.
    ...
    # MODIFIED: Print updated k-mer list
    ...
    for item in tqdm(data_loader, total=total_files, desc="Collecting k-mer frequencies"):
        ...
        for seq in data_df['junction_aa'].dropna():
            # MODIFIED: Extract all k-mers, including those of lengths in specific_kmers
            k_values = set(k_list)
            k_values.update(len(kmer) for kmer in specific_kmers)
        ...
        if file_count % batch_size == 0:
            # MemoryMonitor.log_memory(f"After {file_count} files") # Assuming MemoryMonitor is defined elsewhere
            gc.collect()

    # Filter to keep only frequent k-mers AND include specific k-mers
    frequent_kmers = {kmer for kmer, count in global_kmer_counts.items() if count >= min_kmer_count}
    # MODIFIED: Add specific k-mers to the final feature set
    frequent_kmers.update(specific_kmers)

    print(f"\n[K-mer Encoding] Total unique k-mers: {len(global_kmer_counts)}")
    print(f"[K-mer Encoding] Frequent k-mers (count >= {min_kmer_count}): {len(frequent_kmers) - len(specific_kmers)} + {len(specific_kmers)} specific kmers.")
    # print(f"[K-mer Encoding] Memory reduction: {(1 - len(frequent_kmers)/len(global_kmer_counts))*100:.1f}%") # Original calculation uses original total unique kmers
    ...
    # MODIFIED: Recompute k_values needed for encoding
    k_values_for_encoding = set(k_list)
    k_values_for_encoding.update(len(kmer) for kmer in specific_kmers)
    
    for item in tqdm(data_loader, total=total_files, desc=f"Encoding {sorted(list(k_values_for_encoding))}-mers"):
        if os.path.exists(metadata_path):
            rep_id, data_df, label = item
        else:
            filename, data_df = item
        ...
        kmer_counts = {}
        for seq in data_df['junction_aa'].dropna():
            for k in k_values_for_encoding:
                for i in range(len(seq) - k + 1):
                    kmer = seq[i:i + k]
        ...
        if file_count % batch_size == 0:
            gc.collect()
            # MemoryMonitor.log_memory(f"After encoding {file_count} files") # Assuming MemoryMonitor is defined elsewhere
    ...
    # MemoryMonitor.log_memory("After encoding complete") # Assuming MemoryMonitor is defined elsewhere
...
class ImmuneStatePredictor:
    ...
    def fit(self, train_dir_path: str):
        ...
        if self.specific_kmers:
            print(f"[Training] Specific k-mers included: {self.specific_kmers}")

        # Load and encode k-mers with combined k values
        # MODIFIED: Pass specific_kmers to the encoding function
        X_train_df, y_train_df = load_and_encode_kmers_combined(
            train_dir_path, 
            k_list=self.k_list,
            min_kmer_count=self.min_kmer_count,
            specific_kmers=self.specific_kmers
        )
    ...
    def predict_proba(self, test_dir_path: str) -> pd.DataFrame:
        ...
        # MemoryMonitor.log_memory("After prediction") # Assuming MemoryMonitor is defined elsewhere
    ...
    def identify_associated_sequences(self, train_dir_path: str, top_k: int = 50000) -> pd.DataFrame:
        ...
        k_values_for_scoring = set(len(kmer) for kmer in self.model.feature_names_)
...
def main():
    parser = argparse.ArgumentParser(
        description="Dataset 2 Reproduction Script ",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""
    Example usage:
    python Dataset2_reproduce.py \\
        --train_dir /path/to/train_dataset_2 \\
        --test_dirs /path/to/test_dataset_2_1 /path/to/test_dataset_2_2 \\
    ...
    K_MERS = [3, 4]  
    SPECIFIC_5_MERS = [
    'ADYST', 'AGGLP', 'AGGSN', 'AGHEQ', 'AIEKL', 'AISES', 'AKNIQ', 'ALPYE', 'ANEQF', 'APKET',
    'AQSGE', 'ARDNR', 'ARDRP', 'ARFVT', 'ARTGF', 'ASASV', 'ASKGQ', 'ASRER', 'ASRGL', 'ASRRT',
    'ASSDT', 'ASSEL', 'ASSFL', 'ASSGL', 'ASSHT', 'ASSIR', 'ASSIS', 'ASSKW', 'ASSLE', 'ASSLF',
    'ASSLG', 'ASSLN', 'ASSLR', 'ASSPD', 'ASSPG', 'ASSPH', 'ASSPK', 'ASSPR', 'ASSQE', 'ASSQG',
    'ASSQI', 'ASSQP', 'ASSSE', 'ASSSP', 'ASSSS', 'ASSST', 'ASSSV', 'ASSSW', 'ASSYS', 'ASVQG',
    'AYEQY', 'CAISE', 'CARFV', 'CASAS', 'CASRE', 'CASRG', 'CASRR', 'CASSD', 'CASSE', 'CASSF',
    'CASSG', 'CASSH', 'CASSI', 'CASSK', 'CASSL', 'CASSP', 'CASSQ', 'CASSS', 'CASSY', 'CSARD',
    'CSART', 'CSASK', 'CSVDG', 'CSVRQ', 'DEQYF', 'DFRGE', 'DGGDT', 'DGPNY', 'DISVG', 'DNRAK',
    'DRPRQ', 'DRPSY', 'DTARD', 'DTQYF', 'DTYEQ', 'DYSTD', 'EAYEQ', 'EERNE', 'EGQQY', 'EGWYN',
    'EKLFF', 'ELGRE', 'ENTEA', 'ERGGF', 'ERNEQ', 'ESGRG', 'ESGTG', 'ESVGK', 'ETQYF', 'EWVGT',
    'FGWDE', 'FLTTG', 'FNEQF', 'FRGET', 'FTYEQ', 'FVTGA', 'GAGHE', 'GAIEK', 'GAPKE', 'GAYEQ',
    'GDTYE', 'GELFF', 'GETQY', 'GFNEQ', 'GFTYE', 'GGDTY', 'GGFNE', 'GGGGP', 'GGGPY', 'GGLPE',
    'GGPYN', 'GGQET', 'GGRAL', 'GGSNQ', 'GGSYG', 'GGVNN', 'GHEQY', 'GKYGT', 'GKYSP', 'GLAGG',
    'GLANE', 'GLLYE', 'GLNTE', 'GLPET', 'GNYGY', 'GPNYG', 'GPYNE', 'GQETQ', 'GQGAY', 'GQQYF',
    'GRALP', 'GREGW', 'GRGAG', 'GRGTD', 'GRTYY', 'GSNQP', 'GSYGY', 'GTDTQ', 'GTGLL', 'GTSYN',
    'GTYTQ', 'GVNNE', 'GVYNS', 'GWDEQ', 'GWYNE', 'HEQYF', 'HRDRP', 'HTPIE', 'IEKLF', 'IENTE',
    'ILAQS', 'IRQGV', 'ISESV', 'ISLNT', 'ISQTG', 'ISVGE', 'KETQY', 'KGQGA', 'KNIQY', 'KQGGQ',
    'KRAGG', 'KWESG', 'KYGTY', 'KYSPL', 'LAGGL', 'LANEQ', 'LAQSG', 'LEERN', 'LFGWD', 'LGGRA',
    'LGREG', 'LLYEQ', 'LNNRD', 'LNTEA', 'LPETQ', 'LPYEQ', 'LRGGG', 'LTIYF', 'LTTGG', 'LYEQY',
    'NEQFF', 'NIQYF', 'NNEQF', 'NNRDG', 'NQPQH', 'NRAKN', 'NRDGP', 'NSPLH', 'NTEAF', 'NWLTI',
    'NYGYT', 'PDFRG', 'PETQY', 'PGRGA', 'PGTSY', 'PHRDR', 'PIENT', 'PKETQ', 'PKRAG', 'PNYGY',
    'PQGAI', 'PQNWL', 'PRILA', 'PRQSY', 'PSYEQ', 'PYEQY', 'PYNEQ', 'QEAYE', 'QETQY', 'QEWVG',
    'QGAIE', 'QGAYE', 'QGGQE', 'QGGSY', 'QGRTY', 'QGVYN', 'QISQT', 'QNWLT', 'QPGTS', 'QPQHF',
    'QSGEL', 'QSYNE', 'QTGNY', 'RAGGS', 'RAKNI', 'RALPY', 'RDGPN', 'RDNRA', 'RDRPR', 'RDRPS',
    'REGWY', 'RERGG', 'RFVTG', 'RGAGH', 'RGETQ', 'RGGFN', 'RGGGG', 'RGLAN', 'RGTDT', 'RILAQ',
    'RNEQF', 'RPRQS', 'RPSYE', 'RQEAY', 'RQGVY', 'RQSYN', 'RRTGL', 'RSGKY', 'RTGFT', 'RTGLN',
    'RTQHF', 'RTYYE', 'SARDR', 'SARTG', 'SASKG', 'SASVQ', 'SDTAR', 'SELGR', 'SESGT', 'SESVG',
    'SFLTT', 'SGELF', 'SGKYS', 'SGLAG', 'SGRGT', 'SGTGL', 'SHTPI', 'SIRQG', 'SISLN', 'SKGQG',
    'SKWES', 'SLEER', 'SLFGW', 'SLGGR', 'SLNNR', 'SLNTE', 'SLRGG', 'SNQPQ', 'SPDFR', 'SPGRG',
    'SPHRD', 'SPKRA', 'SPLHF', 'SPQNW', 'SPRIL', 'SQEWV', 'SQGGS', 'SQISQ', 'SQPGT', 'SQTGN',
    'SRERG', 'SRGLA', 'SRRTG', 'SRTQH', 'SSDTA', 'SSELG', 'SSESG', 'SSFLT', 'SSGLA', 'SSHTP',
    'SSIRQ', 'SSISL', 'SSKWE', 'SSLEE', 'SSLFG', 'SSLGG', 'SSLNN', 'SSLRG', 'SSPDF', 'SSPGR',
    'SSPHR', 'SSPKR', 'SSPQN', 'SSPRI', 'SSQEW', 'SSQGG', 'SSQIS', 'SSQPG', 'SSRTQ', 'SSSES',
    'SSSPQ', 'SSSRT', 'SSSSR', 'SSSSW', 'SSSTK', 'SSSTR', 'SSSVA', 'SSSWD', 'SSSWP', 'SSTKQ',
    'SSTRS', 'SSVAD', 'SSWDI', 'SSWPQ', 'SSYST', 'STDTQ', 'STEGQ', 'STKQG', 'STRSG', 'SVADY',
    'SVDGG', 'SVGET', 'SVGKY', 'SVQGR', 'SVRQE', 'SWDIS', 'SWPQG', 'SYEQY', 'SYGYT', 'SYNEQ',
    'SYSTE', 'TARDN', 'TDTQY', 'TEAFF', 'TEGQQ', 'TGAPK', 'TGFTY', 'TGGVN', 'TGLLY', 'TGLNT',
    'TGNYG', 'TKQGG', 'TPIEN', 'TRSGK', 'TSYNE', 'TTGGV', 'TYEQY', 'TYTQY', 'TYYEQ', 'VADYS',
    'VDGGD', 'VGETQ', 'VGKYG', 'VGTDT', 'VNNEQ', 'VQGRT', 'VRQEA', 'VTGAP', 'VYNSP', 'WDEQY',
    'WDISV', 'WESGR', 'WLTIY', 'WPQGA', 'WVGTD', 'WYNEQ', 'YEQYF', 'YGTYT', 'YGYTF', 'YNEQF',
    'YNSPL', 'YSPLH', 'YSTDT', 'YSTEG', 'YTQYF', 'YYEQY']
...
"""
    python Dataset2_reproduce_no_important_seq.py \
        --train_dir /Users/quack/projects/airr/adaptive-immune-profiling-challenge-2025/train_datasets/train_datasets/train_dataset_2 \
        --test_dirs /Users/quack/projects/airr/adaptive-immune-profiling-challenge-2025/test_datasets/test_datasets/test_dataset_2 \
