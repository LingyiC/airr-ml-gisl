...
Usage:
    python Dataset7_reproduce.py --train_dir /path/to/train --test_dirs /path/to/test1 /path/to/test2 --out_dir /path/to/output --n_jobs 4
"""
...
def run_esm_grid_search(esm_path, dataset_name, master_labels, n_jobs=1):
    ...
    # ESM variants
    esm_variants = {
        "BertMax_RowSum": os.path.join(agg_dir, f"aggregated_esm2_t6_8M_max/esm2_{dataset_name}_aggregated_sum.pkl"),
        ...
        "BertMean_RowSum": os.path.join(agg_dir, f"aggregated_esm2_t6_8M_mean/esm2_{dataset_name}_aggregated_sum.pkl"),
...
def run_reproduce_prediction(train_dir: str,
                            ...
                            n_jobs: int = 1,
                            kmer_path: str = None,
                            esm_path: str = None) -> pd.DataFrame:
    """
    Run reproduction prediction by retraining the ensemble model.
    ...
    if kmer_path is None:
        kmer_dir = os.path.join(DEFAULT_OUT_DIR, "kmer")
    ...
    if esm_path is None:
        esm_path = os.path.join(DEFAULT_OUT_DIR, "aggregates")
    ...
    for test_dir in test_dirs:
        ...
        if X_esm is not None and esm_model is not None:
            # Load ESM for test - determine row aggregation type
            if 'RowSum' in best_esm_config['variant']:
                row_agg = 'sum'
            elif 'RowMean' in best_esm_config['variant']:
                row_agg = 'mean'
            else:  # RowMax
                row_agg = 'max'
