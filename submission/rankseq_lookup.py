...
# Configuration - will be set when main() is called


def load_data_generator(
    data_dir: str, metadata_filename="metadata.csv"
    ...
    if os.path.exists(metadata_path):
        ...
        for row in metadata_df.itertuples(index=False):
            ...
            try:
                ...
                print(
                    f"Warning: File '{row.filename}' listed in metadata not found. Skipping."
                )
                continue
    else:
        ...
        for file_path in sorted(tsv_files):
            try:
                ...
                print(
                    f"Warning: Could not read file '{file_path}'. Error: {e}. Skipping."
                )
                continue
...
def load_full_dataset(data_dir: str) -> pd.DataFrame:
    ...
    if os.path.exists(metadata_path):
        ...
        for rep_id, data_df, label in tqdm(
            data_loader, total=total_files, desc="Loading files"
        ):
            data_df["ID"] = rep_id
...
def load_and_encode_kmers_combined(
    data_dir: str,
    k_list: List[int] = [3, 4],
    ...
    file_count = 0
    for item in tqdm(
        data_loader, total=total_files, desc="Collecting k-mer frequencies"
    ):
        if os.path.exists(metadata_path):
    ...
    frequent_kmers = {
        kmer for kmer, count in global_kmer_counts.items() if count >= min_kmer_count
    }
    print(f"\n[K-mer Encoding] Total unique k-mers: {len(global_kmer_counts)}")
    print(
        f"[K-mer Encoding] Frequent k-mers (count >= {min_kmer_count}): {len(frequent_kmers)}"
    )
    print(
        f"[K-mer Encoding] Memory reduction: {(1 - len(frequent_kmers) / len(global_kmer_counts)) * 100:.1f}%"
    )
...
def get_repertoire_ids(data_dir: str) -> list:
    ...
    if os.path.exists(metadata_path):
        ...
        tsv_files = glob.glob(search_pattern)
        repertoire_ids = [
            os.path.basename(f).replace(".tsv", "") for f in sorted(tsv_files)
        ]
...
def validate_dirs_and_files(train_dir: str, test_dirs: List[str], out_dir: str) -> None:
    ...
    assert os.path.isfile(metadata_path), (
        f"`metadata.csv` not found in train directory `{train_dir}`."
    )
...
def concatenate_output_files(out_dir: str) -> None:
    ...
    for pred_file in predictions_files:
        try:
            ...
            print(
                f"Warning: Could not read predictions file '{pred_file}'. Error: {e}. Skipping."
            )
            continue
    ...
    for seq_file in sequences_files:
        try:
            ...
            print(
                f"Warning: Could not read sequences file '{seq_file}'. Error: {e}. Skipping."
            )
            continue
...
class KmerClassifier:
    ...
    def tune_and_fit(self, X, y, val_size=0.0):
        ...
        for C in self.c_values:
            ...
            scores = cross_val_score(
                pipeline, X_train, y_train, cv=cv, scoring=scorer, n_jobs=self.n_jobs
            )
            results.append(
                {
                    "C": C,
        ...
        if X_val is not None:
            if scorer == "balanced_accuracy":
                self.val_score_ = balanced_accuracy_score(
                    y_val, self.model_.predict(X_val)
                )
            else:  # roc_auc
                self.val_score_ = roc_auc_score(
                    y_val, self.model_.predict_proba(X_val)[:, 1]
                )
    ...
    def score_all_sequences(
        ...
        """
        Score all sequences using model coefficients (highly optimized).
        ...
        lookup_sizes = {k: 20**k for k in range(1, max_k + 1)}
        kmer_lookups = {}

        for k in range(1, max_k + 1):
            kmer_lookups[k] = np.full(lookup_sizes[k], -1, dtype=np.float64)

        for kmer, idx in kmer_to_index.items():
            k = len(kmer)
            if k in kmer_lookups:
                code = 0
                valid = True
                for c in kmer:
                    aa_code = AA_TO_INT.get(c, -1)
                    if aa_code < 0:
                        valid = False
                        break
                    code = code * 20 + aa_code
                if valid and 0 <= code < lookup_sizes[k]:
                    kmer_lookups[k][code] = idx

        k_list_arr = np.array(k_list, dtype=np.int32)
        coefficients_arr = coefficients.astype(np.float64)

        lookup_3 = kmer_lookups.get(3, np.array([]))
        lookup_4 = kmer_lookups.get(4, np.array([]))
        lookup_5 = kmer_lookups.get(5, np.array([]))
        lookup_2 = kmer_lookups.get(2, np.array([]))

        try:
            from numba import jit, prange

            @jit(nopython=True, parallel=True, cache=True)
            def _score_sequences_numba(
                sequences,
                k_list,
                lookup_2,
                lookup_3,
                ...
                for idx in prange(n_seqs):
                    ...
                    feature_vec = np.zeros(n_features, dtype=np.float64)
                    seq_len = len(seq)
                    ...
                    for ki in range(len(k_list)):
                        ...
                        for i in range(seq_len - k + 1):
                            ...
                            if valid and 0 <= code < lookup_size:
                                kmer_idx = int(lookup[code])
                                if kmer_idx >= 0:
                                    feature_vec[kmer_idx] = 1.0

                    scores[idx] = np.dot(feature_vec, coefficients)

                return scores

            scores = _score_sequences_numba(
                sequences_list,
                k_list_arr,
                lookup_2,
                lookup_3,
        ...
        try:
            ...
            def _score_sequence(args):
                seq, k_list, kmer_to_index, coefficients, n_features = args
                ...
                for k in k_list:
                    for i in range(max(0, len(seq) - k + 1)):
                        idx = kmer_to_index.get(seq[i : i + k])
                        if idx is not None:
                            feature_vec[idx] = 1.0

                return float(np.dot(feature_vec, coefficients))

            n_workers = min(mp.cpu_count(), 8)
            chunksize = max(1, total_seqs // (n_workers * 4))

            args_list = [
                (seq, k_list, kmer_to_index, coefficients, n_features)
            ...
            with ProcessPoolExecutor(max_workers=n_workers) as executor:
                scores = list(
                    tqdm(
                        executor.map(_score_sequence, args_list, chunksize=chunksize),
                        total=total_seqs,
                        desc=f"Scoring ({n_workers} workers)",
                        unit="seq",
                        unit_scale=True,
                    )
                )
            ...
            for seq in sequences_list[batch_start:batch_end]:
                ...
                for k in k_list:
                    for i in range(max(0, len(seq) - k + 1)):
                        ...
                        if idx is not None:
                            feature_vec[idx] = 1.0
                batch_scores.append(float(np.dot(feature_vec, coefficients)))
...
class ImmuneStatePredictor:
    ...
    def __init__(
        self, k_list=[3, 4], min_kmer_count=2, n_jobs=1, device="cpu", **kwargs
    ):
        """
    ...
    def fit(self, train_dir_path: str):
        ...
        X_train_df, y_train_df = load_and_encode_kmers_combined(
            train_dir_path, k_list=self.k_list, min_kmer_count=self.min_kmer_count
        )
    ...
    def predict_proba(self, test_dir_path: str) -> pd.DataFrame:
        ...
        if self.model.feature_names_ is not None:
            X_test_df = X_test_df.reindex(
                columns=self.model.feature_names_, fill_value=0
            )
    ...
    def identify_associated_sequences(
        ...
        print(
            f"[Sequence Identification] Start time: {start_time.strftime('%Y-%m-%d %H:%M:%S')}"
        )
        ...
        all_sequences_scored = self.model.score_all_sequences(
            unique_seqs, sequence_col="junction_aa", batch_size=500, k_list=self.k_list
        )

        end_time = datetime.now()
        print(
            f"[Sequence Identification] End time: {end_time.strftime('%Y-%m-%d %H:%M:%S')}"
        )
        print(f"[Sequence Identification] Duration: {end_time - start_time}")
...
def _generate_predictions(
    predictor: ImmuneStatePredictor, test_dirs: List[str]
) -> pd.DataFrame:
...
def _save_predictions(predictions: pd.DataFrame, out_dir: str, train_dir: str) -> None:
    ...
    preds_path = os.path.join(
        out_dir, f"{os.path.basename(train_dir)}_test_predictions.tsv"
    )
...
def _save_important_sequences(
    predictor: ImmuneStatePredictor, out_dir: str, train_dir: str
) -> None:
    ...
    seqs_path = os.path.join(
        out_dir, f"{os.path.basename(train_dir)}_important_sequences.tsv"
    )
...
def main(
    ...
    predictor = ImmuneStatePredictor(
        k_list=k_list, min_kmer_count=min_kmer_count, n_jobs=n_jobs, device=device
    )
...
def run():
    ...
    parser = argparse.ArgumentParser(description="RankSeq Lookup")
    parser.add_argument(
        "--train_dir", required=True, help="Path to training data directory"
    )
    parser.add_argument(
        "--test_dirs",
        required=True,
    ...
    parser.add_argument(
        "--n_jobs", type=int, default=1, help="Number of CPU cores to use"
    )
    parser.add_argument(
        "--device",
        type=str,
